...
1. 서점 직원이 서점 본사에 필요한 책 상품을 주문한다.
2. 주문한 내역이 해당 서점 본사에 전달되면 상품을 배송해준다.
3. 배송한 상품을 입고 처리한다.
4. 고객이 상품을 구매한다.
5. 구매 상품에 대한 상품 재고를 변경 처리한다.
6. 고객이 상품 구매를 취소하면 재고가 변경된다.
7. 서점 직원이 주문 내용, 상품 재고 현황을 조회할 수 있다.
8. 서점 직원이 주문, 배송, 상품 재고, 구매 현황들을 알림으로 받을 수 있다.
...
```
- 서점 직원이 상품을 주문 한다. => 완료
- 본사가 주문된 상품을 배송 한다. => 완료
- 배송이 완료되면 상품을 입고처리 한다. => 완료
- 상품 주문시 서점 직원은 view를 통해 주문 상세 내역, 상품 재고현황을 조회할 수 있다. => 완료
```
